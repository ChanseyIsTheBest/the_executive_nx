...
void nxp_update(void) {
  ...
  /* A chord fires when every button in the mask is held and one of them went
   * down this frame; with a single-bit mask that is just "pressed". */
  const u64 cur_mask  = s_cfg.toggle_cursor_mask ? s_cfg.toggle_cursor_mask
                                                 : (u64)HidNpadButton_Plus;
  const u64 gyro_mask = s_cfg.toggle_gyro_mask   ? s_cfg.toggle_gyro_mask
                                                 : (u64)HidNpadButton_Minus;
  #define NXP_CHORD(m) ((m) != NXP_TOGGLE_OFF && ((held & (m)) == (m)) && ((pressed) & (m)))

  if (NXP_CHORD(cur_mask)) {
    s_visible = (s_visible > 0) ? 0 : 1;
    logf_("nxp: cursor %s\n", s_visible ? "ON" : "OFF");
  }
  if (NXP_CHORD(gyro_mask)) {
    if (s_mouse_connected) {
  ...
  if (s_visible > 0) {
    HidAnalogStickState ls = padGetStickPos(&s_pad, s_cfg.cursor_stick ? 1 : 0);
    if (ls.x || ls.y) {
