 ...
 *
 * Both are overridable with toggle_cursor_mask / toggle_gyro_mask below; a
 * port whose game already uses those buttons needs them somewhere else.
...
#define NXP_TOGGLE_OFF ((u64)~0ull)   /* a mask that can never be satisfied */
...
typedef struct {
  ...
  /* Button masks for the two toggles. 0 => the defaults above ('+' and '-').
   *
   * A mask with several bits is a CHORD: it fires when every button in it is
   * held and one of them was pressed this frame. So ZL|ZR means "both
   * shoulders together", not "either shoulder".
   *
   * Set a mask to NXP_TOGGLE_OFF to disable that toggle entirely -- worth
   * doing for the gyro one in a port that already uses the gyro for
   * gameplay, where a second consumer of the same sensor just fights the
   * first. */
  u64   toggle_cursor_mask;
  u64   toggle_gyro_mask;

  int   cursor_stick;            /* 0 => left (upstream default), 1 => right.
                                  * A port whose gameplay uses a stick for
                                  * something else needs the cursor on the
                                  * other one; sharing makes both feel broken. */
